...
packet_data(PosData) ->
  ...
  LAT = trunc(abs(PosData#pos_data.lat) / 90 * 16#FFFFFFFF),
  LON = trunc(abs(PosData#pos_data.long) / 180 * 16#FFFFFFFF),
  ...
  Direction = trunc(PosData#pos_data.dir),
...
response(?EGTS_SR_POS_DATA, <<NTM:?UINT,
LAT:?UINT,
LON:?UINT,
ALTE:1, LOHS:1, LAHS:1, MV:1, BB:1, CS:1, FIX:1, VLD:1,
SPDL:8,
...
DIN:?BYTE,
SRC:?BYTE>> = _Data) ->
  <<Speed:14>> = <<SPDH:6, SPDL:8>>,
  <<Direction:9>> = <<DIRH:1, DIR:8>>,
  RDATA = {
    {time, NTM},
    {lat, LAT},
    {lon, LON},
